...
// abstract over action : creating new functions
...
// can have funcitons that changes other funcitons
...
// we can even write dunctions that provide new type of flow control
...
//  how filter function work in javascript
...
// how to use filter(built-in) in javascript
...
let rtlSrcipts = SCRIPTS.filter(s=>s.direction == "rtl");
// console.log(map(rtlSrcipts, s => s.name));


// how reduce work in javascript
...
// using reduce to find out the script with maximun number of characters
...
// compose operations in javascript using reduce,filter, and map

function average(array){
    return array.reduce((a,b)=>a+b) / array.length;
}

// console.log(Math.round(average(SCRIPTS.filter(
//     s => s.living).map(s => s.year))));
// console.log(Math.round(average(SCRIPTS.filter(
//         s => !s.living).map(s => s.year))));


// using the a function to find out the script of a code
...
// counting abstrcaction recognizing text
...
// console.log(testScripts('英国的狗说"woof", 俄罗斯的狗说"тяв"'));


// flattening  the curve

// let arrays = [[1,2,2],[4,5],[6]];

// console.log(arrays.reduce((a,b)=> a.concat(b)));

// loop

function loop(n, condition, update, log){
    for(let index = n; condition(index);index = update(index)){
        log(index);
    }
}
...
function dominantDirection(text){
    let total = countBy(text, char => {
        let script = characterScript(char.codePointAt(0));
        return script ? script.direction: "none"
    }).filter(({name})=> name != "none");

    if(total.length == 0) return "ltr";

    return total.reduce((a,b)=> a>b?a:b).name;
}