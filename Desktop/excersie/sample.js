...
// console.log([[125184, 125259], [125264, 125274], [125278, 125280]].reduce((count, [from, to])=>{
//     console.log(count);
//     return count + (to-from);
// },0))
...
// console.log(characterScript(25));

function countByEven(items, groupName){
  let counts = [];
  for(let item of items){
    let name = groupName(item)? "even":"odd";
    // console.log(name);
    console.log(counts.findIndex(c=>c.name == name));
    let known = counts.findIndex(c=>c.name == name); // findIndex returns -1 when the index in not found in the array
    if (known == -1){
...
// console.log(countByEven([1,3,4,6,2], n => n%2==0));
...
// console.log(countBy([1,3,4,6,2], n => n>0));
